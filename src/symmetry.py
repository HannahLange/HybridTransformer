...
"""
Implementation adapted from Zhang et al. (https://journals.aps.org/prb/abstract/10.1103/PhysRevB.107.075147)
"""
...
class Symmetry:
    ...
    def apply_random(self, tensor):
        ...
        #tensor[idx==1] = 1-tensor[idx==1]
        #tensor[idx==3] = 1-tensor[idx==3]
        if self.spin_inv_symm:
...
class Symmetry1D(Symmetry):
    ...
    def translation(self, phase=1):
        ...
        self.phase = torch.outer(self.phase, phase_i.to(self.phase.device)).reshape(-1)
...
class Symmetry2D(Symmetry):
    ...
    def translation_x(self, phase=1):
        ...
        perm = torch.stack([perm[:,0], perm[:,2]], axis=1)

        self.permutation = perm.reshape(-1, nx, ny)  # (batch * nx, nx, ny)

        phase_i = phase ** torch.arange(nx)  # (nx, )
        self.phase = torch.outer(self.phase, phase_i.to(self.phase.device)).reshape(-1)  # (batch * nx)
    ...
    def rotation_90(self, phase=1):
        ...
        #self.permutation = torch.cat([perm,perm_4, perm_5], dim=0)  # (batch * 4, nx, ny)
        self.phase = torch.cat([self.phase,
                                phase * self.phase,
                                phase ** 2 * self.phase,
                                phase ** 3 * self.phase,
                                phase ** 4 * self.phase,
                                phase ** 4 * self.phase])  # (batch * 4)

    def rotation_180(self, phase=1):
        """
        perm: (batch, nx, ny), the permutation to be rotated.
...
class Symmetry2D_psi:
    def __init__(self, nx, ny):
        ...
        self.r90_idx = bin2dec(1-basis_r90.reshape(-1, nx*ny), nx*ny).to(torch.int64).cpu().numpy()
        self.inversion_idx = bin2dec(1 - basis.reshape(-1, nx*ny), self.n).to(torch.int64).cpu().numpy()
        self.mirrdiag = bin2dec(torch.flip(basis, [0, 1]))
    ...
    def diag_mirror(self,psi):
        psi = psi[self.mirrdiag]
        return psi
